...
exports.Get_Info_University_For_Parent = async (req, res, next) => {
    try {
        let pool = await sql.connect(Config);
        //let facultys = await pool.request().query("SELECT * from University");
        ...
        let facultys = await pool.request()
            ...
            .query("select u.TenTruongDH,u.WebSite, u.Email,u.SDT,u.FanFage, STUFF((select '; '+ u2.TenDiaChi from University u1, Address_Uni u2 where u1.MaTruong=u2.MaTruong FOR XML PATH('')), 1, 1, '') [TenDiaChi], f.TenKhoa, uf.Website,u.Images from University u, University_Faculty uf, Faculty f where uf.MaTruong=u.MaTruong and uf.MaKhoa=f.MaKhoa and uf.ID = (select i.IDTruongKhoa  from InfoSinhVien i where i.Email= (select i2.Email from InfoSinhVien i2, InfoPhuHuynh ph where i2.ID = ph.IDSinhVien and ph.IDSignin =@IDSignin))");
...
exports.Get_News_University_For_Parent = async (req, res, next) => {
    async function autoScroll(page, finishTime) {
        await page.evaluate(async (finishTime) => {
    ...
    try {
        ...
        let facultys = await pool.request()
            .input('IDSignin', sql.VarChar, req.userData._id)
            .query("select u.news,u.MaTruong from InfoSinhVien i, University_Faculty uf, University u where i.IDTruongKhoa =uf.ID and uf.MaTruong = u.MaTruong and i.Email = (select i2.Email from InfoSinhVien i2, InfoPhuHuynh ph where i2.ID = ph.IDSinhVien and ph.IDSignin =@IDSignin)")
...
exports.Get_News_Faculty_For_Parent = async (req, res, next) => {
    async function autoScroll(page, finishTime) {
        await page.evaluate(async (finishTime) => {
    ...
    try {
        ...
        let facultys = await pool.request()
            .input('IDSignin', sql.VarChar, req.userData._id)
            .query("   select uf.news,uf.MaTruong, uf.MaKhoa from InfoSinhVien i,University_Faculty uf where uf.ID= i.IDTruongKhoa and i.Email= (select i2.Email from InfoSinhVien i2, InfoPhuHuynh ph where i2.ID = ph.IDSinhVien and ph.IDSignin =@IDSignin)")
