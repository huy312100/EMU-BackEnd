...
exports.Client_Socket = async (req, res, next) => {
    async function autoScroll(page, finishTime) {
        await page.evaluate(async (finishTime) => {
    ...
    try {
        var a = await getMainPageNews(req.body.url);
        console.log(a);
        if (a !== undefined) {
            res.status(200).json(a);
        }
                ...
                Account.find({ username: req.body.username })
                    ...
                    .then(re2 => {
                        ...
                        if (re2.length >= 1) {
                            ...
                            chat.find({ "User": { $all: [req.userData.username.toString(), req.body.username.toString()] } })
                                .exec()
                                .then(re3 => {
                                    //console.log(re3[0]);
                                    if (re3.length >= 1) {
                                        res.status(200).json(re3[0]);
                            ...
                            res.status(500).json({err:"err3"});
                            
                        }
                    })
            } else {
                res.status(500).json({err:"err4"});
                //socket.emit("Reply-Create-Room", "error");
            }
        })
};
